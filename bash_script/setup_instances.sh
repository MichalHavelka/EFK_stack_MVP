...
## SETUP EFK_STACK
ssh -o StrictHostKeychecking=no -i ../../key.pem admin@$efk_stack_ec2_2 "sudo apt update && sudo apt install -y rsync net-tools dnsutils htop prometheus-node-exporter"
...
## SETUP FORWARDER
ssh -o StrictHostKeychecking=no -i ../../key.pem admin@$forwarder_ec2_3 "sudo apt update && sudo apt install -y rsync net-tools dnsutils htop prometheus-node-exporter"
...
## SETUP KAFKA
ssh -o StrictHostKeychecking=no -i ../../key.pem admin@$kafka_ec2_1 "sudo apt update && sudo apt install -y rsync net-tools dnsutils htop prometheus-node-exporter"
...
## SETUP MONITORING
rsync -av --mkpath -e "ssh -i ../../key.pem" --rsync-path="sudo rsync" setup_monitoring/* admin@${forwarder_ec2_3}:/opt/monitoring/
ssh -o StrictHostKeyChecking=no -i ../../key.pem admin@${forwarder_ec2_3} "sudo docker compose -f /opt/monitoring/docker-compose.yml up -d"
